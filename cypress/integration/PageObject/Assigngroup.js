...
class assigngroup {
  selectAdmin() {
    access.pimqa();
    const Admin = cy
      .get("#raul-left-navigation-items", { timeout: 2000 })
      .contains("Admin")
      .click({ force: true });
    const IvenstMent = cy
      .get("[class='raul-left-navigation-item-display']")
      .contains("Investment Model");
  ...
  createGroup() {
    ...
    const addGroup = cy
      .get("[class='col mt-3  mb-5']")
      .get("[id='addEntityGroup']")
      .contains("Add Group");
    ...
    testData.forEach((testDataRow) => {
      ...
      context(`Generating a test for ${data.groupname}`, () => {
        ...
        groupName.within(() => {
          cy.get("div")
            .get("[placeholder='Investment Group Name']")
            .type(data.groupname);
        });
      });
  ...
  addEntity() {
    ...
    testData.forEach((testDataRow) => {
      ...
      context(`Generating a test for ${data.groupname}`, () => {
        const entityName = cy
          .get("[class='form-group']")
          .get("[placeholder='Investment Entity Name']");
  ...
  assignEntityGroup() {
    ...
    testData.forEach((testDataRow) => {
      ...
      testData.forEach((testDataRow) => {
        ...
        context(`Generating a test for ${data.groupname}`, () => {
          const ivestGroup = cy
            .get("[class='heading text-charcoal-50 ng-star-inserted']")
            .contains("Assign Investment Group")
            .click({ force: true });
  ...
  assert() {
    const alert = cy
      .get(".raul-success-notification")
      .get("[class='raul-notification-title']");
